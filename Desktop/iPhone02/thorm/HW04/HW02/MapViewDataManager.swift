...
class MapViewDataManager: NSObject {
    ...
    override init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(self.saveLocationHistoryInternal), name: NSNotification.Name(rawValue: "saveModelData"), object: nil)
    }
    
    func saveLocationHistoryInternal() {
        saveLocationHistory(locationHistory: self.locationHistory)
    }
    
    func initLocationHistory() -> CLLocation? {
        if let savedHistory = fetchLocationHistory() {
            self.locationHistory = savedHistory
            NotificationCenter.default.post(name: NSNotification.Name(rawValue: "modelUpdateLocationHistory"), object: nil, userInfo: ["dataKey": savedHistory])
            if savedHistory.count > 0 {
                return savedHistory.last
            } else {
                return nil
            }
        } else {
